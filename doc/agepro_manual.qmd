...
please refer to the [AGEPRO Reference Manual](./AGEPRO_Reference_Manual_v4-25.pdf).
:::
...
* [Stochastic Data Files](#stochastic-data-files)
    * [Stochastic data file format layout](#stochastic-data-file-format-layout)
    * [Time variance](#time-variance)
    * [Stochastic data file format layout with multi-fleets](#stochastic-data-file-format-layout-with-multi-fleets)
...
* [Stock of Age Distribution and Auxiliary Files Output](#stock-of-age-distribution-and-auxiliary-files-output-summaryauxoutputflag)
    * [Summary Report of Stock Numbers of Age](#summary-report-of-stock-numbers-of-age-\.xx1)
    * [Multi-Fleet Auxiliary Data File](#multi-fleet-auxiliary-data-file-\.xx10)    
    * [Switching between `AGEPRO VERSION 4.0` and `AGEPRO VERSION 4.25` Input File Formats](#switching-between-agepro-version-4\.0-and-agepro-version-4\.25-input-file-formats)
...
![](img/01_general_newCase.png){fig-align="left"}

For new cases, the user must supply the following input:

- **First year in Projection**
- **Last year in Projection**
- **First Age Class** (Age 1 [default] or Age 0)
- **Last Age Class**
- **Number Of Population Simulations** (1000 by Default)  
- **Number Of Fleets** (1 by Default)
- **Number Of Recruitment Models** (1 by Default)
- **Random Number Seed**
- (Optional) Enabling **_Discards are Present_** will include [Discard Fraction](#discard-fraction) and [Discard Weights at Age](#discard-weights-at-age) when the new case model is set.

Click **SET** button to proceed.

![](img/01_general_modelOptions.png "General Options"){fig-align="left"}
...
![](img/03_saveINPvalidation_saved.png)

If the validator catches an error or finds invalid input, it will interrput the input file save process, and a dialog window will print out that validation error found. **All invalid input has be resolved before the data may be saved.** 

![](img/03_saveINPvalidation_errorBio.png)

![](img/03_saveINPvalidation_interruptSave.png)
...
Clicking **Launch AGEPRO Model** under the run menu, will allow AGEPRO GUI input and **Bootstrap** data to be passed to the AGEPRO calculation engine. 
 
Before the Calcualtion Engine is execuited, AGEPRO-GUI will check if the GUI inputs are valid. If they are, it will create a new **AGEPRO Model Output Run directory**. GUI input and bootstrap data gets written to file in the **AGEPRO Model Output Run directory** before the AGEPRO calcuation engine launches.

There are three methods AGEPRO-GUI collects the bootstrap filepath: The filepath string from the bootstrap parameter. If that is invalid, then it will assume that the bootsrap file (`*.BSN`) is in the same directory as the AGEPRO Input File. It that is also invalid, then the program allow the user to explictly locate the bootstrap file via the Open File Dialog. If the user cancels from the open file dialog, this will terminate the launch to the calcuation engine and GUI input will be *not* be written to the **AGEPRO Model Output Run directory**.

![](img/20_launch_model.png){width=70%}
...
The **AGEPRO Model Output Run directory** is titled two ways:

- The base path name in _**input file**_ from **general options** (without file extensions) appended with a date-time stamp when the model was launched. 
- If _**input file**_ in **general options** is blank, then it will have `untitled` and the date-time stamp this model launched.
...
A stochastic data file represents data for a single stochastic parameter (example: JAN-1 Stock Weights at Age).
...
In AGEPRO, the user specifies the **Number of Bootstrap Iterations** on the [Bootstrap](#bootstrap) panel, and the **number of (population) simulations** on the [General Options](#creating-a-new-case) panel.  
...
![JAN-1 Time Varying](img/04_jan1_usrTimeVarying.png "JAN-1 Time Varying")
...
![JAN-1 Non Time-Varying](img/04_jan1_usrNonTimeVarying.png "JAN-1 Non Time-Varying")
...
![JAN-1 Read Weights From File](img/04_jan1_file.png "JAN-1 Read Weights From File")
...
Use JAN-1 Weights At Age      | [Uses **Jan-1 Stock Weights at Age** data and CV](#jan-1-stock-weights-at-age)
...
Read Weights from File        | Read the Weights at age with Stochastic error from an external file
Use JAN-1 Weights At Age      | [Uses **Jan-1 Stock Weights at Age** data and CV](#jan-1-stock-weights-at-age)
Use SSB Weights At Age        | [Uses **Spawning Stock Weights at Age** data and CV](#spawning-stock-weights-at-age)
...
Read Weights from File       | Read the Weights at age with Stochastic error from an external file
Use JAN-1 Weights At Age     | [Uses **Jan-1 Stock Weights at Age** data and CV](#jan-1-stock-weights-at-age) and apply to all fleets
Use SSB Weights At Age       | [Uses **Spawning Stock Weights if Age** data and CV](#spawning-stock-weights-at-age) and apply to all fleets
Use Mid-Year Weights At Age  | [Uses **Mid-Year Stock Weights at Age** data and CV](#mid-year-stock-weights-at-age) and apply to all fleets
...
Use SSB Weights At Age       | [Uses **Spawning Stock Weights if Age** data and CV](#spawning-stock-weights-at-age) and apply to all fleets
Use Mid-Year Weights At Age  | [Uses **Mid-Year Stock Weights at Age** data and CV](#mid-year-stock-weights-at-age) and apply to all fleets
Use Catch Weights At Age     | [Uses **Catch Weights at Age** data and CV](#catch-weights-at-age) and apply to all fleets
...
### Example: Time Varying
![User Specified Natural Mortaility, Time Varying](img/05_natmort_usr-timeVarying.png)
...
### Example: Non Time Varying
![User Specfied Natural Mortality without Time Varying](img/05_natmort_usr-nonTimeVarying.png)
...
### Example: From File Option
![Natural Mortality From File](img/05_natmort_file.png)
...
### Example: Time Varying
![User Specfied Maturity, With Time Varying](img/06_maturity_usr-timeVarying.png)
...
### Example: Non Time Varying
![User Specified Matruity, without Time Varying](img/06_maturity_usr-nonTimeVarying.png)
...
### Example: From File Option
![Maturity from File](img/06_maturity_file.png)
...
![Fraction Mortality without Time Varying](img/07_zfrac_nonTimeVarying.png)

In the above example, the user applies the same constant value to all years in the projection horizon.

![Fraction Mortality Prior to Spawn, Time Varying](img/07_zfrac_timeVarying.png)
...
### Example: Time Varying
![User Specfied Fishery Selectivity for each fleet at age, time varying](img/08_fishery_usr-timeVarying.png)
...
### Example: Non Time Varying
![User Specfied Fishery Selectivity for each fleet at age, without time varying](img/08_fishery_usr-nonTimeVarying.png)
...
### Example: From File Option
![Fishery Selectivity from file](img/08_fishery_file.png)
...
![Disabled Discards Fraction parameter](img/09_discard_disable.png)
...
### Example: Time Varying
![User Specfied Discard Fractions per fleet at Age, Time Varying](img/09_discard_usr-TimeVarying.png)
...
### Example: Non Time Varying
![User Specfied Discard Fractions per fleet at Age, non Time Varying](img/09_discard_usr-nonTimeVarying.png)
...
### Example: From File Option
![Discard Fraction from File](img/09_discard_file.png)
...
![Bootstrap Panel](img/10_bootstrap.png){width=95%}
...
The AGEPRO Calculation Engine does not support recruitment ages beyond **age 1**. 
...
::: {.callout-warning}
**VPA(Virtual Population Analysis)** and **ASAP(Age Structured Assessment Program)** are Legacy programs to generate bootstrap files for AGEPRO. 
:::
...
![Harvest Scenario from Acadian Redfish Example 1 Model, 2004-2009](img/13_harvest.png)

In addition, the user may use this data grid for additional harvest calculations such as the **P-Star Analysis**, or to **Apply Rebuilder Target**. By default, **None (only Harvest Scenario)** is selected.
...
* **F-MULT**: Total Fishing Mortality
...
AGEPRO-GUI allows an option for the user to specify a **Rebuilder Target Year** and calculate the Fishing Mortality level required to meet this **Rebulider Target** to a specified **Rebuilder Percent Confidence**.  
...
### Example
![Single Fleet Harvest Scenario Table with a Rebuild Target Example](img/11_rebuilder.png "Harvest Scenario Table w/ Rebuilder Target Options")
...
Select the **Peform P-Star Analysis** option for *Additional calculations* box in the **Harvest Scenario** panel. This will show the _P-Star Specfication_ directly below. 
The _P-Star Specification_ allows the user to specify the **Number of P-Star levels**, **Overfishing F** criteria, **P-Star Target Year**, and **P-Star Levels**. For **P-Star levels**, values should be entered in ascending order from 1 to the max level.
...
### Example
![Multi-Fleet Harvest Scenario Table in a P-Star Analysis Scenario Example](img/12_pstar.png "Harvest Scenario Table w/ P-Star Analysis Attributes")
...
The **Misc. Options** panel includes options users can use to set AGEPRO calcuation engine output. 

![Misc Options Panel](img/15_miscOptions.png "Misc Options Panel on the AGEPRO-GUI")

For **Misc Options** panel, the user can adjust these options:

* [General Ouptut options](#output-options)
* [Stock of Age Distribution and Auxiliary Files Output](#stock-of-age-distribution-and-auxiliary-files-output-summaryauxoutputflag)
* [Retrospective Adjustment Factors](#retrospective-adjustment-factors)
* [Program to View AGEPRO Output (\*.out) file](#view-agepro-output-files)
* [Specifying Bounds](#specifying-bounds)
* [Reference Point Threshold Report](#reference-point-threshold-report)
...
# Stock of Age Distribution and Auxiliary Files Output (SummaryAuxOutputFlag)
...
There are five options for generating Stock Numbers at Age summary report and auxiliary files. The first two options are compatible with `AGEPRO VERSION 4.0` 
and `AGEPRO VERSION 4.25` input file formats. The last three options are only compatible with `AGEPRO VERSION 4.25` input file format.

### 0. No Stock of Age, but Output Auxilary Files EXCEPT Stock Numbers Of Age 
Auxiliary files created, excuding the Stock Numbers of Age summary report. This is the behavior that `AGEPRO VERSION 4.0` and 
`AGEPRO VERSION 4.25` input files will have by default.  

### 1. Output Stock of Age and ALL Auxilary Files                             
This Will Output Stock Numbers at Age and Auxiliary files. This is compatible with `AGEPRO VERSION 4.0` and `AGEPRO VERSION 4.25` 
input files that have the option.  

### 2. NO Stock of Age and Auxilary Files
No Stock Numbers at Age summary report or Auxiliary files will be created. This is only compatible with `AGEPRO VERSION 4.25`.

### 3. Output Stock of Age, but NO Auxilary Files
This option produces a population size at age summary in the output file, but does not generate the auxiliary files. 
This is only compatible with `AGEPRO VERSION 4.25`. 

### 4. Output Stock of Age and Auxilary Files, EXCEPT Stock Numbers Of Age 
This option produces a population size at age summary in the output file, generates the auxiliary files, 
but does not produce a population size at age file summary. This is only compatible with `AGEPRO VERSION 4.25`.
...
:--------------| :-----------------------------------------------------
XX1            | [Stock Numbers of Age](#summary-report-of-stock-numbers-of-age-\.xx1). If **ALL Auxiliary Files** (StockSummaryAuxFlag=1) is enabled
XX2            | Recruits (Fish)
...
XX8            | Discards (MT)
XX9            | F-Mult
XX10           | [Catch by Fleet](#multi-fleet-auxiliary-data-file-\.xx10). `AGEPRO VERSION 4.0`: Additional auxilary file if model is multi-fleet. `AGEPRO VERSION 4.25` and later: Included with an enabled auxilary file option.


## Summary Report of Stock Numbers of Age (`*.XX1`)

Selecting **Output Stock of Age and ALL Auxilary Files** generates auxiliary file XX1, stock numbers at age, in addition to the other files mentioned in the previous section [*Auxilliary File Names*](#auxiliary-file-names).

When enabled, the program will generate a table of stock number distribution at age for each year in the time horizon. The file has a number of columns equal to the number of age classes and a number of rows equal to the number of years in the time horizon multiplied by the number of observed values.

:::{.callout-warning}
**Please Note**:  Generating a summary report with a large time horizon and/or large number of age classes/observed values is not recommended, since it can affect completeion of the AGEPRO model run.
:::

##  Multi-Fleet Auxiliary Data File (`*.XX10`)

For `AGEPRO VERSION 4.0`, If there is more than one Fleet then an additional auxiliary file with file extension XX10 will be created. In `AGEPRO VERSION 4.25` and later, this file is created when any of the auxilary file options are enabled.`

The number of columns will be equal to the number of years in the time horizon.
...
## Switching Between `AGEPRO VERSION 4.0` and `AGEPRO VERSION 4.25` Input File Formats
The **Enable AGEPRO VERSION 4.0 Input File Format** check box allows the user to switch loaded model between the `AGEPRO VERSION 4.0` and `AGEPRO VERSION 4.25` input file format.
The text next to the check box indicates which format the model is currently using. This will also reflect the value of **Input Version File Format** shown in [General](#creating-a-new-case).

When the **Enable AGEPRO VERSION 4.0 Input File format** is checked, AGEPRO input files will be written in the `AGEPRO VERSION 4.0` format. In addition to this,
options in [**Stock of Age Distribution and Auxiliary Files Output**](#stock-of-age-distribution-and-auxiliary-files-output-summaryauxoutputflag) will be limited to the first two options. The program will check the model if the Auilary Flag is set to 
other three options exists ("2. No Stock of Age and Auxilliary Files", "3. Output Stock of Age, but NO Auxilary Files", "4. Output Stock of Age and Auxilary Files, EXCEPT Stock Numbers Of Age") 
before saving to `AGEPRO VERSION 4.0` format.

Unchecking **Enable AGEPRO VERSION 4.0 Input File Format** allows the program to write AGEPRO input files in the `AGEPRO VERSION 4.25` format. 
In addition to this, all five options in [**Stock of Age Distribution and Auxiliary Files Output**](#stock-of-age-distribution-and-auxiliary-files-output-summaryauxoutputflag) are available and vaild.
...
:::{.callout-note}
The program to view AGEPRO Output files and **File Explorer** with the AGEPRO output directory may launch behind the AGEPRO-GUI window. 
:::
...
![Windows system Dialog for selecting appilcation for \*.out fiie](img/13_outFile_default.png "File association"){fig-align="left"}
...
<!--# Link to AGEPRO Projection Samples at examplesDocs.qmd --> 

{{< include ../examples/exampleDocs.qmd >}}